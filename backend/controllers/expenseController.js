...
exports.addExpense = async (req, res, next) => {
    try {
        ...
        let parsedExpense;
        try {
            parsedExpense = JSON.parse(req.body.data);
        } catch (err) {
            return res.status(400).json({ message: 'Invalid JSON in form-data field `data`.' });
        }

        const { amount, category, description, splits } = parsedExpense;
...
exports.deleteExpense = async (req, res, next) => {
    ...
    try {
        ...
        await pool.query(`DELETE FROM expense_comments WHERE id = $1`, [expenseId]);
...
exports.updateExpense = async (req, res, next) => {
    ...
    try {
        ...
        const finalReceiptUrl = receiptUrl || existingReceiptUrl;

        await pool.query(
            `UPDATE expenses SET amount = $1, category = $2, description = $3, receipt_url = $4
   WHERE id = $5`,
            [amount, category, description, finalReceiptUrl, expenseId]
        );
...
exports.getCommentCount = async (req, res) => {
    ...
    try {
        const result = await pool.query(
            "SELECT comment_count FROM expense_comment_counts WHERE expense_id = $1",
            [expenseId]
        );

        if (result.rows.length === 0) {
            return res.json({ expenseId, commentCount: 0 });
        }

        res.json({ expenseId, commentCount: result.rows[0].comment_count });
    } catch (err) {
        console.error(err);
...
exports.getExpenseShare = async (req, res, next) => {
    const { expenseId } = req.params;
    const userId = req.user.id;
...
exports.deleteComment = async (req, res, next) => {
    ...
    try {
        ...
        const result = await pool.query(
            `SELECT * FROM expense_comments WHERE id = $1`,
            [commentId]
        );
        ...
        // Only allow the commenter or group admin to delete (optional)
        if (comment.user_id !== userId) {
            return res.status(403).json({ message: 'Not authorized to delete this comment' });
        }

        // Delete the comment
        await pool.query(`DELETE FROM expense_comments WHERE id = $1`, [commentId]);
